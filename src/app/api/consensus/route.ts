...
export async function POST(request: Request) {
  ...
  if (!Array.isArray(ids) || ids.length < 2) {
    return NextResponse.json(
      { error: "Provide an array of at least 2 analysis IDs." },
      { status: 400 },
    );
  }
  ...
  if (analyses.length < 2) {
    return NextResponse.json(
      { error: "Could not find enough analyses. Ensure the IDs exist." },
      { status: 404 },
    );
  }

  try {
    const result = await buildConsensus(analyses, apiKey);
