...
hard_data_points: You are a forensic signal extractor. Your job is to find two categories of insight from this transcript:

CATEGORY 1 — NUMERICAL CLAIMS: Find EVERY number spoken — no exceptions. Work through the transcript from beginning to end, timestamp by timestamp. Every time you see a number — a dollar amount, a percentage, a time duration, a count, a multiplier (e.g. "10x"), a rank, a ratio, a measurement — stop and extract it as a data point. Do NOT editorialize about whether a number is "important enough." If a speaker said it with a number attached, extract it.

CATEGORY 2 — QUALITATIVE BUSINESS THESES: Also extract as data points any high-conviction thesis the speaker makes about a named company, investment, person, or business model — even if no number is directly attached. These are often the most valuable insights in the video and are the ones most commonly missed. Examples of what to extract:
  - A speaker explaining WHY a set of companies or investments were chosen (the specific thesis connecting them)
  - A speaker identifying the structural advantage of a business model
  - A speaker's conviction argument about a named strategy or market dynamic that has no number attached but contains clear business logic

GROUPING RULE (critical): If the speaker discusses multiple named companies or investments as part of ONE overarching thesis or strategy, group them into a SINGLE data point — do NOT create a separate data point per company. Example: if a speaker discusses Costco, Amazon, and Berkshire Hathaway as the three positions in a concentrated portfolio, that is ONE data point. The metric_title states the overarching thesis. The speaker_thesis then covers what each company specifically contributes to that thesis in one cohesive explanation. Only split into separate data points if the speaker treats each company as a completely independent argument in different parts of the video.

For each qualitative thesis data point: metric_title should state the thesis as a complete, specific claim (NOT "Nick Sleep invested in three stocks" — instead: "Nick Sleep's concentrated 3-stock portfolio thesis: Costco, Amazon, and Berkshire Hathaway each embody the same scaled-economics model — where more customers directly lower costs for all customers").
...
ANTI-REPETITION MANDATE: Each field must serve a UNIQUE purpose. If you catch yourself writing the same idea in different words across multiple fields, stop and rewrite. The purpose of each field is non-negotiable:
  ...
  - speaker_thesis: Extracts THE LESSON (what does this data point reveal? — no restatement of the quote)
...
If a speaker names a dollar amount, a time figure paired with money, or any specific number used to make an argument, it is a hard data point. Extract it. For EACH one return these fields:
  ...
  - speaker_thesis: KEY INSIGHT — what does this data point REVEAL? Do NOT summarize what was said. Interpret: what does this data point tell a founder or investor about the speaker's business model, market dynamics, or strategic position? 3-4 sentences written as a business analyst, not a narrator. Use the speaker's actual name.
    SPECIAL RULE — Named Company or Investment Data Points: If this data point references multiple named companies or investments as part of one thesis, explain what each company specifically contributes to the overarching argument IN A SINGLE COHESIVE EXPLANATION — do not write a separate paragraph per company. Focus on the shared structural principle the speaker identified across all of them, then briefly explain how each company embodies that principle differently. Do NOT write generic observations like "concentrated investment requires deep understanding." Instead explain the specific mechanism: what did the speaker identify as the shared structural characteristic, and how does each named company express it? Example: instead of "Sleep's portfolio shows high conviction in three companies," write "Sleep's thesis centres on businesses where the act of serving more customers structurally lowers the cost of serving each one — Costco passes margin savings directly to customers rather than shareholders, Amazon reinvests revenue into infrastructure that reduces fulfillment costs at scale, and Berkshire compounds capital through insurance float at near-zero cost. The unifying logic is that all three businesses become harder to compete against the larger they grow, which is the opposite of most companies where scale increases bureaucracy and erodes margin."
