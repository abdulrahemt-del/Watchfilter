...
CATEGORY 2 — QUALITATIVE BUSINESS THESES: Also extract as data points any high-conviction thesis the speaker makes about a named company, investment, person, or business model — even if no number is directly attached. These are often the most valuable insights in the video and are the ones most commonly missed. Examples of what to extract:
  - A speaker explaining WHY Costco succeeds (the specific pricing philosophy, customer relationship, competitive moat)
  - A speaker explaining WHY Berkshire Hathaway is a long-term hold (the specific compounding mechanism, capital allocation thesis)
  - A speaker identifying the structural advantage of a business model (e.g., "Costco returns margin savings to customers rather than maximizing gross margin")
  - A speaker's conviction thesis about a named company, strategy, or market dynamic that has no number attached but contains clear business logic
For each such qualitative thesis: metric_title should state the thesis as a complete, specific claim (NOT "Nick Sleep invested in Costco" — instead: "Costco's structural inversion of retail economics — returning margin savings to customers — creates compounding market share gains competitors cannot replicate through advertising").
...
If a speaker names a dollar amount, a time figure paired with money, or any specific number used to make an argument, it is a hard data point. Extract it. For EACH one return these fields:
  ...
  - speaker_thesis: KEY INSIGHT — what does this data point REVEAL? Do NOT summarize what was said. Interpret: what does this data point tell a founder or investor about the speaker's business model, market dynamics, or strategic position? 3-4 sentences written as a business analyst, not a narrator. Use the speaker's actual name.
    SPECIAL RULE — Named Company or Investment Data Points: If this data point references a specific named company (e.g., Costco, Amazon, Berkshire Hathaway) or named investment thesis, do NOT write generic observations like "concentrated investment requires deep understanding" or "high-conviction strategy." Instead explain the SPECIFIC business model, competitive moat, pricing philosophy, or structural advantage the speaker attributes to that entity. Ask: what does THIS company do differently that the speaker identified? What is the mechanism — not the conclusion? Example: instead of "Sleep's portfolio shows high conviction," write "Sleep identifies Costco's structural inversion of retail economics — where margin savings are returned to customers rather than captured as profit — as the self-reinforcing flywheel behind its market share gains. Unlike traditional retailers who compete on brand or advertising, Costco's model means that growing revenue automatically deepens its pricing advantage, making customer loyalty a structural outcome rather than a marketing achievement."
